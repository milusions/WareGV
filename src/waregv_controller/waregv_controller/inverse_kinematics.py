...
class InverseKinematics(Node):
    def __init__(self):
        ...
        # Define speed limits: 50 RPM converted to rad/s
        self.max_rpm = 50.0
        self.max_rad_s = self.max_rpm * (2 * np.pi) / 60.0  # Approx 5.236 rad/s
        
        self.get_logger().info(f'IK Node Started. Max Speed Capped at: {self.max_rpm} RPM ({self.max_rad_s:.2f} rad/s). Output: [RF, RR, LF, LR]')
    ...
    def inverse_kinematics(self, linear_cmd, angular_cmd):
        ...
        # Cap/Clamp each motor's velocity to the max allowed rad/s limit
        omega_l = np.clip(omega_l, -self.max_rad_s, self.max_rad_s)
        omega_r = np.clip(omega_r, -self.max_rad_s, self.max_rad_s)
