...
bool STS3215Driver::init(const std::string & port_name, int baud_rate, const std::vector<uint8_t> & servo_ids)
{
  ...
  if (tcsetattr(serial_fd_, TCSANOW, &options) != 0) {
    std::cerr << "[STS3215Driver] Failed to set serial attributes." << std::endl;
    close_port();
    return false;
  }

  // Automatically configure all target servos to continuous rotation (Wheel) mode
...
bool STS3215Driver::read_response(uint8_t id, std::vector<uint8_t> & rx_data, size_t expected_bytes)
{
  ...
  size_t total_expected = expected_bytes + 6; 
